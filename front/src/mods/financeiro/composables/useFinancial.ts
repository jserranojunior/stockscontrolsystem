...
import { dateFormatUs } from "alvitre-obelisk/src/convertDates/us/convertDateToUs";

export const useFinancial = () => {
  const router = useRouter();

  async function GetEditBillsToPay(
    id: number,
  ...
  function setContasAPagar(res: any) {
    if (res) {
      store.ContaAPagar = res;
    }
  }

  async function getCategoriaContas(
    selectedDate: string,
    FuncHttpFinanceiro: any = useHttpResources
  ) {
  ...
  function setCategoriaTest() {
    store.categoriaContas = {
  ...
  function formaterBillsToPay(bills: any) {
  ...
  function setDataCalendario(data: string) {
    if (data) {
      store.Calendario.selectedDate = data;
    }
  }

  async function storeBillsToPay(
    data: any = store.ContaAPagar,
    FuncHttpFinanceiro: any = useHttpResources
  ) {
    let useHttpFinanceiro = FuncHttpFinanceiro();
    ...
    if (
      !store.ContaAPagar.favorecido ||
      !store.ContaAPagar.inicio_data_pagamento
    ) {
  ...
  async function deleteBillPayment(
    ...
    return await useHttpFinanceiro.delet(id).then(() => {
      return true;
    });
  }

  async function setEditAddMode(mode: string) {
    store.editAddMode.mode = mode;
    return mode;
  }

  async function updateBillsToPay(
    contasAPagar = store.ContaAPagar,
    FuncHttpFinanceiro: any = useHttpResources
  ) {
    const urlApi = `/financial/billstopay/${contasAPagar.ID}/${contasAPagar.ValoresContasAPagar.data_pagamento}`;
    ...
    if (!contasAPagar.favorecido || !contasAPagar.inicio_data_pagamento) {
      ...
      const billsFormated = formaterBillsToPay(contasAPagar);
      return await useHttpFinanceiro
  ...
  async function setValuesFormBillsToPay() {
    ...
    store.inicioDataPagamentoWithouFormated = dateFormatUs(setDate);
  }

  async function makeBillPayment(
    id: number,
    data_pagamento: any = store.Calendario.selectedDate,
    FuncHttpFinanceiro: any = useHttpResources
  ) {
    let useHttpFinanceiro = FuncHttpFinanceiro();
    const data = {
